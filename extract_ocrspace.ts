...
async function processImages() {
  ...
  for (const url of urls) {
    try {
      ...
      const response = await fetch(`https://api.ocr.space/parse/imageurl?apikey=helloworld&url=${encodeURIComponent(url)}&language=eng&isOverlayRequired=false`);
      ...
      if (data.ParsedResults && data.ParsedResults.length > 0) {
        ...
        console.log(`No text found for ${url}`);
      }
    } catch (e: any) {
  ...
  fs.writeFileSync('ocr_space_results.json', JSON.stringify(results, null, 2));
  console.log('Done!');
}
